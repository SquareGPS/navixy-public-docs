...
The [X-GPS Tracker app](https://x-gps.app) transforms a smartphone or tablet into a GPS device, providing real-time tracking for field staff such as drivers, couriers, service technicians, construction workers, sales representatives, and more. Using a combination of satellite navigation signals (GPS), GSM, and Wi-Fi networks, the app accurately determines the device owner’s location while maintaining low energy consumption depending on settings.

Additionally, the app lets field employees report their location, status, and task completion in real time. Users can share their location, upload photos, and fill out forms, so companies can track workforce progress and manage field tasks.
...
{% columns %}
{% column %}
The new device appears in Navixy with **Just registered** status waiting for the employee to complete the activation.

The employee receives an invitation via text (SMS) or email depending on setup. The invitation includes:
...
If you encounter any issues, consult with your [service provider](about-service-providers.md).

{% hint style="info" %}
If your employee has issues, have them try hitting the **Apply Settings** option from the email.
{% endhint %}
