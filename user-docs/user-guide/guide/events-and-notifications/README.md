---
description: >-
  Set up GPS alert rules to detect geofence breaches, driver safety events, fuel
  changes, and device status. Get notified via SMS, email, push, or in-app.
---
...
Click on an event in the list to see the following information:
...
To access the notification rules setup in Navixy, go to the left menu and choose **Alerts**. This opens the Notification panel. Click on **Set rules**, and the Alert rules interface appears.

![](../../../.gitbook/assets/image-20240805-214736.png)

In the **Rules** interface, you can:

* **View existing rules**: The interface displays a list of all existing alert rules.
...
* **Rule settings**: For each rule, you can configure the following settings:
  ...
  * **Notifications**: Configure how and to whom the notifications are sent.
  * **Schedule**: Set the schedule for when the rule is active.
...
* **Select objects**: In the **Rule settings** panel, you see a list of available objects on the left side. These objects can be individual vehicles or groups of vehicles organized by departments.
...
* **Rule type**: Select one of the many event types that can be monitored with the Navixy platform.
...
The notification text here helps identify specifically what alert was triggered. The notification message here is what appears when triggered.
...
* **Active periods**: The blue blocks represent the times when the rule is active and triggers notifications. You can set these blocks for specific hours of the day and days of the week.
* **Inactive periods**: The gray blocks indicate times when the rule is inactive and doesn't trigger notifications. This helps avoid unnecessary alerts during non-operational hours.

To customize the schedule, click on the desired time blocks to toggle between active and inactive states. This flexibility ensures that you receive notifications only during relevant times, reducing the chances of alert fatigue and enhancing the effectiveness of your monitoring efforts.
...
When a vehicle or asset equipped with a GPS tracking device crosses the boundary of a geofence, an event is triggered. The Navixy platform then logs this event and sends notifications to the user according to the predefined settings.
...
3. **Set notification preferences**:
   1. Choose the channels through which notifications are sent, such as SMS, email, push notifications, or in-app alerts.
...
By using car anti-theft alerts, organizations can significantly enhance the security of their vehicles, quickly respond to potential theft, and minimize the risk of asset loss.
...
When you [activate](../quick-start/activate-gps-device.md) a new device on the Navixy platform, the system automatically creates some rules based on the device’s capabilities, such as a **Pressing SOS Button** rule for a personal GPS device with an emergency button. This automation saves you time by setting up essential monitoring features right from the start, ensuring that your device is ready for use immediately.
