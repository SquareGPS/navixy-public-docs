...
description: API calls to import circle geofences from a preloaded TSV file.
---
...
* `type` - string. Import type, always `zone`.
...
* `progress` - object. Import progress.
  * `imported` - int. Count of successfully imported geofences.
...
```sh
curl -X POST "https://api.eu.navixy.com/v2/zone/import/start" \
...
```sh
curl -X POST "https://api.eu.navixy.com/v2/zone/import/read" \
    -H "Content-Type: application/json" \
...
```sh
curl -X POST "https://api.eu.navixy.com/v2/zone/import/list" \
    -H "Content-Type: application/json" \
...
```sh
curl -X POST "https://api.eu.navixy.com/v2/zone/import/download_failed" \
    -H "Content-Type: application/json" \
    -d '{"hash": "a6aa75587e5c59c32d347da438505fc3", "process_id": 7}'
```
...
```sh
curl -X POST "https://api.eu.navixy.com/v2/zone/import/finish" \
    -H "Content-Type: application/json" \
    -d '{"hash": "a6aa75587e5c59c32d347da438505fc3", "process_id": 7}'
```
